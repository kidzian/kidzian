    ...
    <div className="w-full min-h-screen bg-white flex flex-col items-center gap-16 p-6 md:p-24">
      ...
      >
        {cardData.map((card, index) => (
          <motion.div
            key={index}
            className="bg-white h-60 md:h-[48vh] rounded-lg p-6 shadow-lg flex flex-col items-center text-center cursor-pointer"
            whileHover={{ scale: 1.1, boxShadow: '0px 8px 20px rgba(0, 0, 0, 0.2)' }}
          >
            {card.image}
            <h1 className="text-black text-lg font-semibold mt-4">{card.title}</h1>
            <p className="text-sm text-gray-500 mt-2">{card.paragraph}</p>
            <a
              href={card.redirectLink}
              onClick={() => handleCardClick(card.buttonText)}
              className="mt-auto"
            >
              <button className="bg-gradient-to-r from-purple-600 to-orange-500 text-white px-4 py-2 rounded-lg mt-4">
                {card.buttonText}
              </button>
            </a>
          </motion.div>
